...
def _dlog_check_params(g, p, t, q) :
...
def dlog(dlp) :
	"""
	Compute first 0 <= x <= q-1 such that g^x = t mod p.
	"""
	_dlog_check_params(g, p, t, q)
	(g,p,t,q) = dlp.as_tuple(q=True)
...
def babygiantstep(g, p, t, q) :
	...
	rootq = isqrt(q)
	# include_rootq2 = int(pow(rootq,2) < q)
...
def pohlighellman(g, p, t, qfactors) :
